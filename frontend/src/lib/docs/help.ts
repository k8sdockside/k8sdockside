            ...
            blocks: [
                {
                    ...
                    items: [
                        ...
                        '**Describe** is the same report `kubectl describe` prints, with recent events at the end. **Copy** puts all of it on the clipboard; select a part to copy just that.',
                        'Above the report, the panel lays out what the object’s status says and what it points at: its **Conditions**, coloured by what each one means; **Related** objects — its owner, the node, service account, claims, config maps and secrets a pod uses, a binding’s role, a route’s gateways and backends — each opening in the same panel; and, for anything that selects pods, the **Pods** it selects right now, with their status.',
                        ...
                        '**Actions** depend on the kind: scale, restart and **rollback** to an earlier revision for workloads, and pausing a Deployment’s rollout; **run now** and suspend or resume for cron jobs, and suspend for jobs; **evict** for a pod, which a disruption budget can refuse where delete would not ask; approve or deny for a pending certificate signing request; cordon, uncordon and drain for nodes; delete for anything. Anything that cannot be undone asks first.',
