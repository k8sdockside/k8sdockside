...
| `detectWorkloads` | For a product that installs no custom resources at all: objects whose presence gives it away, as `[{ "kind", "namespace", "selector", "field", "values" }]` — Flannel is found by its DaemonSet, `{ "kind": "daemonsets", "selector": "app=flannel" }`. Any one of them matching is enough. Each needs a `selector`, a `field`, or both; `namespace` is optional (every namespace without it). A `field` recognises an object by what it says rather than how it is labelled: a field path as in `cards`, or one ending in a `{key}` lookup for a key with dots in it. An object matches when the field holds one of `values`, or, with none given, anything but nothing or zero — `{ "kind": "nodes", "field": "status.capacity{nvidia.com/gpu}" }` is a node with NVIDIA GPUs, and `{ "kind": "resourceslices", "field": "spec.driver", "values": ["gpu.nvidia.com"] }` a DRA driver publishing them. The field form needs K8s Dockside 0.1.14 or newer. Unlike `detect`, which is answered from definitions the app has already read, each of these is a real query, so keep them few and narrow. |
